...
The project team consists of four members with defined roles following agile methodologies: a Project Manager, a Product Owner, and a Development Team members.
...
\subsubsection{a. Does all the data come from a single source?}

No. The scenario indicates that sales information comes from the POS System, while promotions come from the Promotion System, and commercial goals are managed in a separate Commercial Goals System. Therefore, the information is distributed across multiple sources.

\subsubsection{b. What challenge does the organization face if information is distributed across several systems?}

The company must integrate data with different structures and formats to obtain a unified and consistent view of the business. Without this integration, analyses may be incomplete or inconsistent.

\subsubsection{c. Can a dashboard answer these questions without integrating multiple data sources?}

No. A dashboard only visualizes available information. If data remains separated across the POS System, the Promotion System, and the Commercial Goals System, it will not be possible to answer questions combining these elements or obtain complete indicators. Data integration is an essential prerequisite.
...
\textbf{Navigation:} A top navigation bar allows switching between the main dashboard, the goal-compliance detail view, and the reports/export section, keeping the interface simple for non-technical managers.
...
The dashboard follows a logical reading flow: filters (define the context) $\rightarrow$ KPIs (overview) $\rightarrow$ charts (deep analysis) $\rightarrow$ details (goal compliance) $\rightarrow$ export (share information).
